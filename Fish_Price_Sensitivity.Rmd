...
# Create Feed Raster
feed_annual_rast <- biomass_rast*1000*feed_conversion_ratio*feed_price # multiplied by 1000 to convert biomass from MT to kg
...
```{r}
# Revenue, Profits, NPV, total NPV as a function
fun_price_fish <- function (price_fish_value) {
  # Revenue
  revenue_rast <- biomass_rast*price_fish_value
  # Profit
  profit_raster <- revenue_rast-cost_of_suitable
  # NPV
  npv <- ((profit_raster/(1-risk_discount)^1) +
...
# Create sequence of prices
price_seq <- seq(from=6.5, to=7.5, by=0.1)

res = sapply(price_seq, function(K) {tmp = mapply(FUN=fun_price_fish, price_fish_value=K); return(tmp)})

apply_fish_df <- t(as.data.frame(do.call(rbind, res)))

colnames(apply_fish_df)=price_seq

resg <- as.data.frame(apply_fish_df) %>% 
  gather(fish_price, sum_npv)

fish_boxplot <- ggplot(resg, aes(x = fish_price, y = sum_npv, fill=fish_price))+
  geom_boxplot() +
  labs(y="Profits", x = "Fish Price") +
  theme_bw()
